...
// ══════════════════════════════════════════════════════════════════
//  MASTRO STRUTTURE — Integrato in MASTRO ERP · DS v2.0
// ══════════════════════════════════════════════════════════════════

const DS = {
  bg: "#F2F1EC", bg2: "#EDECE7", surface: "#FFFFFF",
  text: "#1A1A1C", text2: "#7A7A7A", text3: "#AAAAAA",
  border: "#E5E4DF", borderLight: "#EDECE7",
  accent: "#0D7C6B", accentSoft: "rgba(13,124,107,0.08)", accentMed: "rgba(13,124,107,0.15)",
  blue: "#3B7FE0", blueSoft: "rgba(59,127,224,0.08)",
  green: "#1A9E73", greenSoft: "rgba(26,158,115,0.08)",
  red: "#DC4444", redSoft: "rgba(220,68,68,0.08)",
  orange: "#E8A020", orangeSoft: "rgba(232,160,32,0.08)",
  r: 8, r2: 10,
...
// ═══ MAIN ═════════════════════════════════════════════════════
export default function MastroStrutture({ onClose }) {
  ...
  return <div style={{ width: "100vw", height: "100vh", background: DS.bg, display: "flex", flexDirection: "column", fontFamily: F, color: DS.text, overflow: "hidden", position: "fixed", inset: 0, zIndex: 9999 }}>
    <div style={{ height: 52, background: "#1A1A1C", display: "flex", alignItems: "center", padding: "0 16px", gap: 12, flexShrink: 0 }}>
      {onClose && <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, background: "rgba(255,255,255,0.1)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Ic s={16} c="#fff"><polyline points="15 18 9 12 15 6"/></Ic></button>}
      <div style={{ width: 28, height: 28, borderRadius: 7, background: DS.accent, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#fff", fontSize: 13, fontWeight: 800 }}>M</span></div>
      <span style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>Strutture</span>
      <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{tipo?.n || "Configuratore"}</span>
      <div style={{ flex: 1 }} />
      {st && <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
        {FASI.map((f, i) => { const isA = fase===f.k, isP = FASI.findIndex(x => x.k===fase) > i; return <div key={f.k} style={{ display: "flex", alignItems: "center", gap: 3 }}>
          {i > 0 && <div style={{ width: 12, height: 1, background: isP ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.1)" }} />}
          <button onClick={() => setFase(f.k)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 100, border: "none", cursor: "pointer", background: isA ? DS.accent : "transparent", color: isA ? "#fff" : isP ? "#fff" : "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 500, fontFamily: F }}>
            <span style={{ width: 15, height: 15, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 600, fontFamily: M, background: isA ? "rgba(255,255,255,0.25)" : isP ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.08)", color: isA || isP ? "#fff" : "rgba(255,255,255,0.4)" }}>{isP ? "✓" : f.n}</span>{f.l}
          </button>
        </div> })}
      </div>}
    </div>
...
function FaseTipo({ onSel }) {
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 36, padding: 32 }}>
    <div style={{ textAlign: "center" }}><h1 style={{ fontSize: 30, fontWeight: 700, margin: 0, letterSpacing: "-0.04em" }}>Cosa vuoi progettare?</h1><p style={{ fontSize: 14, color: DS.text3, marginTop: 6 }}>Configura profili, copertura e genera il disegno tecnico</p></div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, maxWidth: 820, width: "100%" }}>
      {TIPI.map(t => <button key={t.id} onClick={() => onSel(t.id)} onMouseEnter={() => setHov(t.id)} onMouseLeave={() => setHov(null)}
        style={{ padding: "22px 14px", borderRadius: DS.r2, cursor: "pointer", border: `1px solid ${hov===t.id ? DS.accent : DS.border}`, background: DS.surface, display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 10, boxShadow: hov===t.id ? DS.shadowMd : DS.shadow, transition: "all 0.2s", transform: hov===t.id ? "translateY(-2px)" : "none", textAlign: "left" }}>
        <div style={{ width: 34, height: 34, borderRadius: 8, background: DS.accentSoft, border: `1px solid ${DS.borderLight}`, display: "flex", alignItems: "center", justifyContent: "center" }}><Ic s={17} c={DS.accent}>{t.icon}</Ic></div>
        <div><div style={{ fontSize: 13, fontWeight: 600 }}>{t.n}</div><div style={{ fontSize: 10, color: DS.text3, marginTop: 2 }}>{t.desc}</div></div>
      </button>)}
    </div>
  </div>
}
...
function FasePensConfig({ st, tipo, up, next }) {
  ...
  return <div style={{ height: "100%", display: "flex" }}>
    <div style={{ width: 300, borderRight: `1px solid ${DS.border}`, padding: 20, display: "flex", flexDirection: "column", gap: 16, overflowY: "auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 30, height: 30, borderRadius: 7, background: DS.bg2, border: `1px solid ${DS.borderLight}`, display: "flex", alignItems: "center", justifyContent: "center" }}><Ic s={15}>{tipo.icon}</Ic></div>
        <div><div style={{ fontSize: 13, fontWeight: 600 }}>Pensilina</div><div style={{ fontSize: 10, color: DS.text3 }}>Fissata a parete, a sbalzo</div></div>
      </div>
      <div style={{ height: 1, background: DS.borderLight }} />

      <div>
        <Label>Dimensioni</Label>
        {[{ k: "larghezza", l: "Larghezza (lungo muro)" }, { k: "sporto", l: "Sporto (profondità)" }, { k: "altFissaggio", l: "Altezza fissaggio a muro" }].map(d => <div key={d.k} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
          <span style={{ fontSize: 10, color: DS.text3, width: 80 }}>{d.l}</span>
          <input type="number" value={p[d.k]} step={50} onChange={e => upP(d.k, parseInt(e.target.value) || 0)} style={{ flex: 1, padding: "6px 8px", borderRadius: 7, border: `1px solid ${DS.border}`, fontSize: 12, fontFamily: M, textAlign: "right", background: DS.bg, outline: "none" }} />
          <span style={{ fontSize: 9, color: DS.text3, fontFamily: M }}>mm</span>
        </div>)}
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <input type="range" min={3} max={30} value={p.pendenza} onChange={e => upP("pendenza", parseInt(e.target.value))} style={{ flex: 1 }} />
          <span style={{ fontSize: 13, fontFamily: M, fontWeight: 600, minWidth: 36 }}>{p.pendenza}%</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {BRACCI.map(b => <button key={b.id} onClick={() => upP("tipoBraccio", b.id)} style={{ padding: "8px 10px", borderRadius: 7, fontSize: 11, fontWeight: 500, cursor: "pointer", border: p.tipoBraccio === b.id ? `1px solid ${DS.accent}` : `1px solid ${DS.border}`, background: p.tipoBraccio === b.id ? DS.accentSoft : DS.bg, color: p.tipoBraccio === b.id ? DS.accent : DS.text2, textAlign: "left", fontFamily: F }}>
      ...
      <div>
        <Label>Opzioni</Label>
        {[{ k: "pannelloSx", l: "Pannello laterale SX" }, { k: "pannelloDx", l: "Pannello laterale DX" }, { k: "grondaFrontale", l: "Gronda frontale" }, { k: "montantiFrontali", l: "Montanti frontali (2 piantoni)" }].map(o => <label key={o.k} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", padding: "5px 0" }}>
          <input type="checkbox" checked={p[o.k]} onChange={e => upP(o.k, e.target.checked)} style={{ accentColor: DS.accent }} />
          <span style={{ fontSize: 11 }}>{o.l}</span>
        </label>)}
      </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {COPERTURE.filter(c => c.id !== "none").map(c => <button key={c.id} onClick={() => up(prev => ({ ...prev, roofType: c.id }))} style={{ padding: "6px 10px", borderRadius: 6, fontSize: 10, fontWeight: 500, cursor: "pointer", border: st.roofType === c.id ? `1px solid ${DS.accent}` : `1px solid ${DS.border}`, background: st.roofType === c.id ? DS.accentSoft : DS.bg, color: st.roofType === c.id ? DS.accent : DS.text2, textAlign: "left" }}>{c.n}</button>)}
        </div>
      </div>
      ...
      <div style={{ flex: 1 }} />
      <Btn primary onClick={next} style={{ width: "100%" }}>Avanti — Profili →</Btn>
    </div>

    {/* SVG Preview sezione laterale */}
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: DS.bg2, position: "relative" }}>
      <div style={{ position: "absolute", top: 14, left: 14, fontSize: 11, color: DS.text3 }}>Sezione laterale pensilina · Vista dal fianco</div>
      ...
      <div style={{ position: "absolute", top: 14, right: 14, display: "flex", gap: 6, zIndex: 10 }}>
        <Btn small active={labelMode} onClick={() => setLabelMode(!labelMode)} style={{ fontSize: 10, background: labelMode ? "#FFFDE7" : DS.surface, border: labelMode ? "1px solid #F59E0B" : `1px solid ${DS.border}`, color: labelMode ? "#92400E" : DS.text2 }}>🏷 Etichetta</Btn>
      </div>
    </div>
  </div>
}
...
function FasePianta({ st, tipo, up, next }) {
  ...
  return <div style={{ height: "100%", display: "flex" }}>
    <div style={{ width: 260, borderRight: `1px solid ${DS.border}`, padding: 18, display: "flex", flexDirection: "column", gap: 14, overflowY: "auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div>
        <Label>Dimensioni</Label>
        {[{ k: "W", l: "Largh." }, { k: "D", l: "Prof." }, { k: "H", l: "Alt." }].map(d => <div key={d.k} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
          <span style={{ fontSize: 10, color: DS.text3, width: 36 }}>{d.l}</span>
          <input type="number" value={st[d.k]} step={50} onChange={e => { const v = parseInt(e.target.value) || 0; up(p => { const rw = d.k === "W" ? v / p.W : 1, rd = d.k === "D" ? v / p.D : 1; return { ...p, [d.k]: v, pianta: p.pianta.map(pt => ({ x: Math.round(pt.x * rw / 50) * 50, y: Math.round(pt.y * rd / 50) * 50 })) } }) }} style={{ flex: 1, padding: "5px 7px", borderRadius: 6, border: `1px solid ${DS.border}`, fontSize: 12, fontFamily: M, textAlign: "right", background: DS.bg, outline: "none" }} />
          <span style={{ fontSize: 9, color: DS.text3, fontFamily: M }}>mm</span>
        </div>)}
      </div>
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
          {[{ k: "rect", svg: "M4,4 L28,4 L28,20 L4,20 Z" }, { k: "L", svg: "M4,4 L28,4 L28,14 L16,14 L16,20 L4,20 Z" }, { k: "U", svg: "M4,4 L28,4 L28,20 L22,20 L22,10 L10,10 L10,20 L4,20 Z" }, { k: "trap", svg: "M8,4 L24,4 L28,20 L4,20 Z" }].map(x => <button key={x.k} onClick={() => setShape(x.k)} style={{ padding: "8px", borderRadius: 6, border: `1px solid ${DS.border}`, background: DS.bg, cursor: "pointer" }}><svg width={28} height={18} viewBox="0 0 32 24"><path d={x.svg} fill="none" stroke={DS.text3} strokeWidth={1.5} /></svg></button>)}
        </div>
      </div>
      <div>
        <Label>Colore</Label>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 4 }}>
          {COLORI.map(c => <button key={c.h} onClick={() => up(p => ({ ...p, colStruct: c.h }))} style={{ width: "100%", aspectRatio: "1", borderRadius: 6, background: c.h, cursor: "pointer", border: st.colStruct === c.h ? `2px solid ${DS.accent}` : `1px solid ${DS.border}` }} />)}
        </div>
      </div>
      {tipo.hasRoof && <div>
        <Label>Copertura</Label>
        {COPERTURE.map(c => <button key={c.id} onClick={() => up(p => ({ ...p, roofType: c.id }))} style={{ display: "block", width: "100%", padding: "5px 8px", marginBottom: 3, borderRadius: 6, fontSize: 10, cursor: "pointer", border: st.roofType === c.id ? `1px solid ${DS.accent}` : `1px solid ${DS.border}`, background: st.roofType === c.id ? DS.accentSoft : DS.bg, color: st.roofType === c.id ? DS.accent : DS.text2, textAlign: "left" }}>{c.n}</button>)}
      </div>}
      {tipo.hasRoof && <div>
        <Label>Pendenza tetto</Label>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><input type="range" min={0} max={30} value={st.tetto.pendenza} onChange={e => up(p => ({ ...p, tetto: { ...p.tetto, pendenza: parseInt(e.target.value) } }))} style={{ flex: 1 }} /><span style={{ fontSize: 12, fontFamily: M, fontWeight: 600 }}>{st.tetto.pendenza}%</span></div>
        <div style={{ display: "flex", gap: 3, marginTop: 4 }}>{["fronte", "retro", "sx", "dx"].map(d => <button key={d} onClick={() => up(p => ({ ...p, tetto: { ...p.tetto, direzione: d } }))} style={{ flex: 1, padding: "4px", borderRadius: 5, fontSize: 9, fontWeight: 600, cursor: "pointer", border: st.tetto.direzione === d ? `1px solid ${DS.accent}` : `1px solid ${DS.border}`, background: st.tetto.direzione === d ? DS.accentSoft : DS.bg, color: st.tetto.direzione === d ? DS.accent : DS.text3, textTransform: "capitalize" }}>{d}</button>)}</div>
      </div>}
      <div style={{ flex: 1 }} />
      <Btn primary onClick={next} style={{ width: "100%" }}>Profili →</Btn>
    </div>
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: DS.bg2, overflow: "auto", position: "relative" }}>
      <svg ref={svgRef} width={Math.max(svgW, 500)} height={Math.max(svgH, 400)} style={{ cursor: dragIdx !== null ? "grabbing" : labelMode ? "crosshair" : "default" }}>
        <defs><pattern id="g50" width={50*sc} height={50*sc} patternUnits="userSpaceOnUse"><path d={`M ${50*sc} 0 L 0 0 0 ${50*sc}`} fill="none" stroke="#E5E5E5" strokeWidth={0.5} /></pattern></defs>
        <rect width="100%" height="100%" fill="url(#g50)" />
        <polygon points={pts.map(p => { const v = toS(p); return `${v.x},${v.y}` }).join(" ")} fill={st.colStruct + "0A"} stroke={st.colStruct} strokeWidth={2} />
        {pts.map((p, i) => { const nx = pts[(i + 1) % pts.length], s1 = toS(p), s2 = toS(nx), mx = (s1.x + s2.x) / 2, my = (s1.y + s2.y) / 2, len = Math.round(Math.sqrt((nx.x - p.x) ** 2 + (nx.y - p.y) ** 2)), nnx = -(s2.y - s1.y), nny = s2.x - s1.x, nl = Math.sqrt(nnx * nnx + nny * nny) || 1, tx = mx + (nnx / nl) * 22, ty = my + (nny / nl) * 22; return <g key={`d${i}`}>
          <rect x={tx - 24} y={ty - 9} width={48} height={18} rx={5} fill={DS.surface} stroke={DS.border} strokeWidth={1} style={{ cursor: "pointer" }} onClick={() => setEditDim({ idx: i })} />
          {editDim?.idx === i ? <foreignObject x={tx - 22} y={ty - 8} width={44} height={16}><input autoFocus type="number" defaultValue={len} style={{ width: "100%", height: "100%", border: "none", background: "transparent", textAlign: "center", fontSize: 10, fontFamily: M, fontWeight: 600, outline: "none" }} onBlur={e => applyDim(i, parseInt(e.target.value))} onKeyDown={e => { if (e.key === "Enter") applyDim(i, parseInt(e.target.value)) }} /></foreignObject>
        ...
        {pts.map((p, i) => { const v = toS(p); return <circle key={`v${i}`} cx={v.x} cy={v.y} r={dragIdx === i ? 8 : 5} fill={dragIdx === i ? DS.accent : DS.surface} stroke={DS.accent} strokeWidth={2} style={{ cursor: "grab" }} onMouseDown={e => { e.preventDefault(); setDragIdx(i) }} onDoubleClick={() => rmPt(i)} /> })}
        <EtichetteLayer etichette={etichette} fase="pianta" onAdd={addLabel} onRemove={removeLabel} onEdit={editLabel} addMode={labelMode} />
      </svg>
      <div style={{ position: "absolute", top: 14, left: 14, display: "flex", gap: 6, zIndex: 10 }}>
        <Btn small active={labelMode} onClick={() => setLabelMode(!labelMode)} style={{ fontSize: 10, background: labelMode ? "#FFFDE7" : DS.surface, border: labelMode ? "1px solid #F59E0B" : `1px solid ${DS.border}`, color: labelMode ? "#92400E" : DS.text2 }}>🏷 Etichetta</Btn>
        {labelMode && <span style={{ fontSize: 10, color: "#92400E", alignSelf: "center" }}>Clicca sulla pianta</span>}
      </div>
    </div>
  </div>
}
...
function FaseProfili({ st, tipo, up, next }) {
  ...
  return <div style={{ height: "100%", display: "flex" }}>
    <div style={{ flex: 1, maxWidth: 440, borderRight: `1px solid ${DS.border}`, padding: 24, overflowY: "auto", display: "flex", flexDirection: "column", gap: 18 }}>
      <div>
        <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Profili strutturali</h2>
        <p style={{ fontSize: 11, color: DS.text3, marginTop: 3 }}>{st.isPens ? "Assegna profilo a ogni componente della pensilina" : "Assegna profilo e togli montanti/travi se necessario"}</p>
      </div>
      ...
      {st.isPens ? <>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            {st.pianta.map((_, i) => { const off = st.montantiOff.includes(i); return <button key={i} onClick={() => toggleMont(i)} style={{ width: 34, height: 34, borderRadius: 7, border: `1.5px solid ${off ? DS.red : DS.green}`, background: off ? DS.redSoft : DS.greenSoft, cursor: "pointer", fontSize: 11, fontWeight: 700, fontFamily: M, color: off ? DS.red : DS.green }}>{off ? "✕" : i + 1}</button> })}
          </div>
        </div>
      </>}

      <div style={{ flex: 1 }} />
      <Btn primary onClick={next} style={{ width: "100%" }}>{st.isPens ? "3D →" : "Lati →"}</Btn>
    </div>

    <div style={{ flex: 1, background: DS.bg2, padding: 28, overflowY: "auto" }}>
      <Label>Riepilogo</Label>
      <div style={{ background: DS.surface, borderRadius: DS.r2, border: `1px solid ${DS.border}`, padding: 18 }}>
        {st.isPens ? <div style={{ fontSize: 12, display: "flex", flexDirection: "column", gap: 8 }}>
...
function FaseLati({ st, tipo, up, next }) {
  ...
  return <div style={{ height: "100%", display: "flex" }}>
    <div style={{ width: 190, borderRight: `1px solid ${DS.border}`, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "12px 12px 6px", fontSize: 10, fontWeight: 600, color: DS.text3, textTransform: "uppercase" }}>Lati · {st.lati.length}</div>
      <div style={{ flex: 1, overflowY: "auto" }}>{st.lati.map((l, i) => <div key={l.id} onClick={() => setSel(i)} style={{ padding: "9px 12px", cursor: "pointer", borderLeft: `2px solid ${sel === i ? DS.accent : "transparent"}`, background: sel === i ? DS.accentSoft : "transparent" }}><div style={{ fontSize: 11, fontWeight: 600 }}>{l.nome}</div><div style={{ fontSize: 9, color: DS.text3, fontFamily: M }}>{l.lunghezza} · {l.elementi.length} el.</div></div>)}</div>
      <div style={{ padding: 12 }}><Btn primary onClick={next} style={{ width: "100%" }}>3D →</Btn></div>
    </div>
    <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "8px 12px", borderBottom: `1px solid ${DS.border}`, display: "flex", gap: 3, alignItems: "center", flexWrap: "wrap" }}>
        <span style={{ fontSize: 10, fontWeight: 600, color: DS.text3, marginRight: 4 }}>INSERISCI</span>
        {ELEMENTI.map(el => <Btn key={el.id} small active={addM === el.id} onClick={() => { setAddM(addM === el.id ? null : el.id); setLabelMode(false) }} style={{ fontSize: 9, padding: "3px 7px" }}>{el.n}</Btn>)}
        <div style={{ width: 1, height: 20, background: DS.border, margin: "0 4px" }} />
        <Btn small active={labelMode} onClick={() => { setLabelMode(!labelMode); setAddM(null) }} style={{ fontSize: 9, padding: "3px 7px", background: labelMode ? "#FFFDE7" : DS.surface, border: labelMode ? "1px solid #F59E0B" : `1px solid ${DS.border}`, color: labelMode ? "#92400E" : DS.text2 }}>🏷 Etichetta</Btn>
      </div>
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: DS.bg2, overflow: "auto" }}>
        <svg width={Math.max(sW + px * 2, 400)} height={Math.max(sH + py * 2, 280)} onClick={e => { if (labelMode || !addM) return; const r = e.currentTarget.getBoundingClientRect(); addEl(addM, Math.round((e.clientX - r.left - px) / sx / 50) * 50); setAddM(null) }} style={{ cursor: addM ? "crosshair" : labelMode ? "crosshair" : "default" }}>
          ...
          {lato.elementi.map(el => { const ed = ELEMENTI.find(e => e.id === el.tipo); const ex = px + el.x * sx, ey = py + el.y * sx, ew = el.w * sx, eh = el.h * sx; return <g key={el.id}>
            ...
            <circle cx={ex + ew - 1} cy={ey + 1} r={5} fill={DS.red} opacity={0.85} style={{ cursor: "pointer" }} onClick={e => { e.stopPropagation(); rmEl(el.id) }} />
            <text x={ex + ew - 1} y={ey + 4} textAnchor="middle" fontSize={7} fill="#fff" fontWeight={600} style={{ pointerEvents: "none" }}>×</text>
          </g> })}
          <text x={px + sW / 2} y={py - 22} textAnchor="middle" fontSize={10} fontFamily={M} fontWeight={600}>{lato.lunghezza}</text>
          <text x={px + sW / 2} y={18} textAnchor="middle" fontSize={12} fontWeight={600}>{lato.nome}</text>
          <EtichetteLayer etichette={etichette} fase={`lato_${sel}`} onAdd={addLabel} onRemove={removeLabel} onEdit={editLabel} addMode={labelMode} />
        </svg>
      </div>
      {lato.elementi.length > 0 && <div style={{ padding: "6px 12px", borderTop: `1px solid ${DS.border}`, display: "flex", gap: 4, overflowX: "auto" }}>
        {lato.elementi.map(el => <div key={el.id} style={{ display: "flex", gap: 3, alignItems: "center", padding: "4px 6px", borderRadius: 6, border: `1px solid ${DS.border}`, background: DS.bg, fontSize: 9 }}>
          <span style={{ fontWeight: 500 }}>{ELEMENTI.find(e => e.id === el.tipo)?.n}</span>
          {["x", "y", "w", "h"].map(k => <input key={k} type="number" value={el[k]} step={50} onChange={e => upEl(el.id, { [k]: parseInt(e.target.value) || 0 })} style={{ width: 38, padding: "2px 3px", borderRadius: 4, border: `1px solid ${DS.border}`, fontSize: 9, fontFamily: M, textAlign: "right", background: DS.bg, outline: "none" }} />)}
          <button onClick={() => rmEl(el.id)} style={{ padding: "1px 4px", borderRadius: 4, border: "none", background: DS.redSoft, color: DS.red, fontSize: 8, fontWeight: 600, cursor: "pointer" }}>✕</button>
        </div>)}
      </div>}
    </div>
  </div>
...
function Fase3D({ st, tipo, next }) {
  ...
  useEffect(() => {
    ...
    const scene = new THREE.Scene(); scene.background = new THREE.Color("#F2F1EC"); scene.fog = new THREE.Fog("#F2F1EC", 15, 35)
    ...
    const gnd = new THREE.Mesh(new THREE.PlaneGeometry(20, 20), new THREE.MeshLambertMaterial({ color: 0xEDECE7 })); gnd.rotation.x = -Math.PI / 2; gnd.receiveShadow = true; scene.add(gnd)
    scene.add(Object.assign(new THREE.GridHelper(10, 20, 0xE5E4DF, 0xEDECE7), { position: { x: 0, y: 0.001, z: 0 } }))
  ...
  return <div style={{ height: "100%", position: "relative" }}>
    ...
    <div style={{ position: "absolute", top: 16, left: 16, display: "flex", gap: 6 }}>
      <Btn small onClick={() => setRot(!rot)}>{rot ? "⏸" : "▶"}</Btn>
      <Btn small active={wf} onClick={() => setWf(!wf)}>{wf ? "◼" : "◻"}</Btn>
      <Btn small primary onClick={next}>Disegno →</Btn>
    </div>
    <div style={{ position: "absolute", bottom: 16, left: 16, padding: "10px 14px", borderRadius: DS.r2, background: "rgba(255,255,255,0.96)", boxShadow: DS.shadowMd }}>
      <div style={{ fontSize: 13, fontWeight: 600 }}>{tipo.n}</div>
...
function FaseDisegno({ st, tipo }) {
  ...
  return <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
    <div style={{ padding: "10px 20px", borderBottom: `1px solid ${DS.border}`, display: "flex", alignItems: "center", gap: 12 }}>
      <span style={{ fontSize: 14, fontWeight: 600 }}>Disegno Tecnico</span><div style={{ flex: 1 }} />
      <Btn small onClick={handlePrint}><Ic s={14} c={DS.text2}>{ICO.print}</Ic> Stampa/PDF</Btn>
    </div>
    <div style={{ flex: 1, overflow: "auto", padding: 24, background: DS.bg2 }}>
      <div ref={printRef} style={{ maxWidth: 900, margin: "0 auto", background: "#fff", padding: 32, borderRadius: DS.r2, boxShadow: DS.shadowLg, border: `1px solid ${DS.border}` }}>
        ...
        {st.isPens ? <>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <svg width={500} height={280} style={{ display: "block", margin: "0 auto" }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <svg width={500} height={200} style={{ display: "block", margin: "0 auto" }}>
              {(() => {
