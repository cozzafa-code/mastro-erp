            ...
            <div style={{ margin: "8px 16px 4px" }}>
              ...
              {curCC && (
                <div style={{ background: T.card, borderRadius: 12, border: `1px solid ${T.bdr}`, padding: "12px 14px" }}>
                  ...
                  {/* ══ PREVENTIVO (CUORE — AREA DI LAVORO) ══ */}
                  {curCC.id === "preventivo" && (
                    <div>
                      <div style={{ fontSize: 11, color: T.sub, marginBottom: 8 }}>{vaniCC.length} vani · {vaniConPrezzoCC.length} con prezzo · Rivedi e modifica prima di inviare</div>
                      
                      {/* Lista vani editabile */}
                      <div style={{ background: T.bg, borderRadius: 8, padding: 8, marginBottom: 8, maxHeight: 200, overflow: "auto" }}>
                        {vaniCC.map((v, vi) => {
                          const pv = calcolaVanoPrezzo(v, c);
                          const lv = v.larghezza || v.l || 0;
                          const hv = v.altezza || v.h || 0;
                          return (
                            <div key={vi} style={{ padding: "6px 0", borderBottom: vi < vaniCC.length - 1 ? `1px solid ${T.bdr}` : "none" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div>
                                  <span style={{ fontSize: 12, fontWeight: 700, color: T.text }}>{v.nome || v.tipo || `Vano ${vi + 1}`}</span>
                                  <span style={{ fontSize: 10, color: T.sub, marginLeft: 6 }}>{v.tipo} · {lv}×{hv} · {v.pezzi || 1}pz</span>
                                </div>
                                <span style={{ fontSize: 13, fontWeight: 800, color: pv > 0 ? T.acc : T.red }}>€{fmtCC(pv)}</span>
                              </div>
                              {/* Dettagli: sistema, colore, vetro, accessori */}
                              <div style={{ fontSize: 9, color: T.sub2, marginTop: 2 }}>
                                {v.sistema || c.sistema || "—"} · {v.colore || "Bianco"} · {v.vetro || "Standard"}
                                {v.accessori?.tapparella?.attivo && " · 🪟 Tapparella"}
                                {v.accessori?.persiana?.attivo && " · 🏠 Persiana"}
                                {v.accessori?.zanzariera?.attivo && " · 🦟 Zanzariera"}
                              </div>
                            </div>
                          );
                        })}
                      ...
                      {/* Voci libere */}
                      {(c.vociLibere || []).length > 0 && (
                        <div style={{ background: T.bg, borderRadius: 8, padding: 8, marginBottom: 8 }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: T.sub, marginBottom: 4 }}>VOCI EXTRA</div>
                          {(c.vociLibere || []).map((vl, vli) => (
                            <div key={"vl" + vli} style={{ display: "flex", justifyContent: "space-between", fontSize: 11, padding: "3px 0" }}>
                              <span style={{ color: T.text }}>{vl.desc || "Voce"} × {vl.qta || 1}</span>
                              <span style={{ color: T.acc, fontWeight: 700 }}>€{fmtCC((vl.importo || 0) * (vl.qta || 1))}</span>
                            </div>
                          ))}
                        </div>
                      )}

                      {/* Aggiungi voce libera inline */}
                      {ccConfirm === "addVoce" ? (
                        <div style={{ background: T.bg, borderRadius: 8, padding: 8, marginBottom: 8, border: `1px solid ${T.bdr}` }}>
                          <input placeholder="Descrizione (es. Posa in opera, Smaltimento)" value={voceTempDesc || ""} onChange={e => setVoceTempDesc(e.target.value)} style={{ width: "100%", padding: 8, borderRadius: 6, border: `1px solid ${T.bdr}`, fontSize: 12, fontFamily: "inherit", boxSizing: "border-box" as any, marginBottom: 4 }} />
                          <div style={{ display: "flex", gap: 4, marginBottom: 6 }}>
                            <input type="number" placeholder="€ Importo" value={voceTempImporto || ""} onChange={e => setVoceTempImporto(e.target.value)} style={{ flex: 2, padding: 8, borderRadius: 6, border: `1px solid ${T.bdr}`, fontSize: 12, fontFamily: "inherit" }} />
                            <input type="number" placeholder="Qtà" value={voceTempQta || "1"} onChange={e => setVoceTempQta(e.target.value)} style={{ flex: 1, padding: 8, borderRadius: 6, border: `1px solid ${T.bdr}`, fontSize: 12, fontFamily: "inherit" }} />
                          </div>
                          <div style={{ display: "flex", gap: 6 }}>
                            <button onClick={() => setCcConfirm(null)} style={{ flex: 1, padding: 8, borderRadius: 8, border: `1px solid ${T.bdr}`, background: T.card, color: T.sub, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Annulla</button>
                            <button onClick={() => {
                              if (!voceTempDesc || !voceTempImporto) return;
                              const nuovaVoce = { id: Date.now(), desc: voceTempDesc, importo: parseFloat(voceTempImporto) || 0, qta: parseInt(voceTempQta) || 1 };
                              setCantieri(cs => cs.map(cm => cm.id === c.id ? { ...cm, vociLibere: [...(cm.vociLibere || []), nuovaVoce] } : cm));
                              setSelectedCM(prev => ({ ...prev, vociLibere: [...(prev.vociLibere || []), nuovaVoce] }));
                              setCcConfirm(null); setVoceTempDesc(""); setVoceTempImporto(""); setVoceTempQta("1");
                            }} style={{ flex: 2, padding: 8, borderRadius: 8, border: "none", background: T.acc, color: "#fff", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>+ Aggiungi</button>
                          </div>
                        </div>
                      ) : (
                        <div onClick={() => setCcConfirm("addVoce")} style={{ padding: "8px 0", textAlign: "center", fontSize: 11, color: T.acc, fontWeight: 700, cursor: "pointer", marginBottom: 4 }}>+ Aggiungi voce extra (posa, smaltimento, ecc.)</div>
                      )}

                      {/* Sconto */}
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, padding: "4px 8px", background: T.bg, borderRadius: 8 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: T.sub, flex: 1 }}>Sconto %</span>
                        {[0, 5, 10, 15, 20].map(s => (
                          <div key={s} onClick={() => {
                            setCantieri(cs => cs.map(cm => cm.id === c.id ? { ...cm, scontoPerc: s } : cm));
                            setSelectedCM(prev => ({ ...prev, scontoPerc: s }));
                          }} style={{ padding: "4px 8px", borderRadius: 6, fontSize: 10, fontWeight: 800, cursor: "pointer", background: (c.scontoPerc || 0) === s ? T.acc : T.card, color: (c.scontoPerc || 0) === s ? "#fff" : T.text, border: `1px solid ${(c.scontoPerc || 0) === s ? T.acc : T.bdr}` }}>{s === 0 ? "No" : s + "%"}</div>
                        ))}
                      </div>

                      {/* IVA selector */}
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, padding: "4px 8px", background: T.bg, borderRadius: 8 }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: T.sub, flex: 1 }}>IVA</span>
                        {[4, 10, 22].map(iv => (
                          <div key={iv} onClick={() => {
                            setCantieri(cs => cs.map(cm => cm.id === c.id ? { ...cm, ivaPerc: iv } : cm));
                            setSelectedCM(prev => ({ ...prev, ivaPerc: iv }));
                          }} style={{ padding: "4px 10px", borderRadius: 6, fontSize: 10, fontWeight: 800, cursor: "pointer", background: ivaPercCC === iv ? T.acc : T.card, color: ivaPercCC === iv ? "#fff" : T.text, border: `1px solid ${ivaPercCC === iv ? T.acc : T.bdr}` }}>{iv}%</div>
                        ))}
                      </div>

                      {/* Totale con sconto */}
                      {(() => {
                        const sconto = c.scontoPerc || 0;
                        const totScontato = totPrevCC * (1 - sconto / 100);
                        const totFinale = totScontato * (1 + ivaPercCC / 100);
                        return (
                          <div style={{ padding: "10px 12px", background: `${T.acc}10`, borderRadius: 10, marginBottom: 8 }}>
                            {sconto > 0 && (
                              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                                <span style={{ color: T.sub }}>Subtotale</span>
                                <span style={{ color: T.sub, textDecoration: "line-through" }}>€{fmtCC(totPrevCC)}</span>
                              </div>
                            )}
                            {sconto > 0 && (
                              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                                <span style={{ color: "#34c759" }}>Sconto {sconto}%</span>
                                <span style={{ color: "#34c759", fontWeight: 700 }}>-€{fmtCC(totPrevCC * sconto / 100)}</span>
                              </div>
                            )}
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 2 }}>
                              <span style={{ color: T.sub }}>Imponibile</span>
                              <span style={{ fontWeight: 700 }}>€{fmtCC(totScontato)}</span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 4 }}>
                              <span style={{ color: T.sub }}>IVA {ivaPercCC}%</span>
                              <span>€{fmtCC(totScontato * ivaPercCC / 100)}</span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", borderTop: `1px solid ${T.bdr}`, paddingTop: 6 }}>
                              <span style={{ fontSize: 13, fontWeight: 800 }}>TOTALE</span>
                              <span style={{ fontSize: 16, fontWeight: 900, color: T.acc }}>€{fmtCC(totFinale)}</span>
                            </div>
                          </div>
                        );
                      })()}
                      ...
                      {/* Note preventivo */}
                      <textarea placeholder="Note per il cliente (condizioni, tempi, garanzie...)" value={c.notePreventivo || ""} onChange={e => {
                        setCantieri(cs => cs.map(cm => cm.id === c.id ? { ...cm, notePreventivo: e.target.value } : cm));
                        setSelectedCM(prev => ({ ...prev, notePreventivo: e.target.value }));
                      }} rows={2} style={{ width: "100%", padding: 8, borderRadius: 8, border: `1px solid ${T.bdr}`, fontSize: 11, fontFamily: "inherit", boxSizing: "border-box" as any, resize: "vertical", marginBottom: 8 }} />

                      {/* Azioni */}
                      <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                        <button onClick={() => generaPreventivoPDF(c)} style={{ flex: 1, padding: 10, borderRadius: 8, border: `1px solid ${T.acc}`, background: `${T.acc}08`, color: T.acc, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>📄 Scarica PDF</button>
                      ...
                      <button onClick={() => {
                        ...
                        window.open(`https://wa.me/${tel.startsWith("39") ? tel : "39" + tel}?text=${encodeURIComponent(`Gentile ${c.cliente}, in allegato il preventivo per la commessa ${c.code}. Totale: €${fmtCC(totIvaCC)} IVA inclusa.`)}`, "_blank");
                        ...
                        setCcDone("📤 Preventivo inviato!"); setTimeout(() => setCcDone(null), 3000);
                      }} style={{ width: "100%", padding: 14, borderRadius: 10, border: "none", background: "#25d366", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit" }}>📤 INVIA PREVENTIVO AL CLIENTE →</button>
                      <div style={{ textAlign: "center", marginTop: 4 }}>
                        <span onClick={() => {
                          setCantieri(cs => cs.map(cm => cm.id === c.id ? { ...cm, preventivoInviato: true, dataPreventivoInvio: new Date().toISOString().split("T")[0] } : cm));
                          setSelectedCM(prev => ({ ...prev, preventivoInviato: true }));
                          setCcDone("✅ Preventivo segnato come inviato"); setTimeout(() => setCcDone(null), 3000);
