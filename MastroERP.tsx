...
// MASTRO ERP v2 — Next.js + Supabase
"use client";
...
/* == HELPER: conta misure compilate di un vano == */
const countMisure = (v: any): number => Object.values(v?.misure || {}).filter((x: any) => (x as number) > 0).length;
...
export default function MastroERP({ user, azienda: aziendaInit }: { user?: any, azienda?: any }) {
  // Derived user info
  const userName = user?.nome || user?.user_metadata?.full_name?.split(" ")[0] || "Utente";
  const userFullName = user?.nome_completo || user?.user_metadata?.full_name || "Utente";
  ...
  const [aiMsgs, setAiMsgs] = useState([{ role: "ai", text: "Ciao! Sono MASTRO AI. Chiedimi qualsiasi cosa sulle tue commesse, task o misure." }]);
  ...
  const advanceFase = (cmId) => {
    const FASE_TEAM = { preventivo: "Sara Greco", conferma: "Sara Greco", misure: "Marco Ferraro", ordini: "Sara Greco", produzione: "Marco Ferraro", posa: "Marco Ferraro", chiusura: userFullName };
    setCantieri(cs => cs.map(c => {
      if (c.id !== cmId) return c;
      const idx = faseIndex(c.fase);
      if (idx < PIPELINE.length - 1) {
        const next = PIPELINE[idx + 1];
        return { ...c, fase: next.id, log: [{ chi: userName, cosa: `avanzato a ${next.nome}`, quando: "Adesso", color: next.color }, ...c.log] };
      }
      return c;
    }));
    if (selectedCM?.id === cmId) {
      ...
      if (idx < PIPELINE.length - 1) {
        const next = PIPELINE[idx + 1];
        const addetto = FASE_TEAM[next.id] || userFullName;
        setSelectedCM(prev => ({ ...prev, fase: next.id, log: [{ chi: userName, cosa: `avanzato a ${next.nome}`, quando: "Adesso", color: next.color }, ...prev.log] }));
  ...
  const exportPDF = () => {
    ...
    html += `<p style="text-align:center;margin-top:30px;color:#999;font-size:11px">Generato da MASTRO ERP — ${new Date().toLocaleDateString("it-IT")}</p></body></html>`;
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: "14px 16px 12px", background: T.card, borderBottom: `1px solid ${T.bdr}` }}>
        ...
        <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.3 }}>
          {(() => { const h = today.getHours(); return h < 12 ? "Buongiorno" : h < 18 ? "Buon pomeriggio" : "Buonasera"; })()}, {userName}
        </div>
              ...
              <div>
                ...
                <div style={{ padding:"0 16px" }} onTouchStart={onSwCmStart} onTouchEnd={onSwCmEnd}>
                  {cmFiltrate.length === 0 ? (
                    ...
                    const vaniMis = vaniA.filter(v => countMisure(v) >= 6).length;
                ...
                {cat?.id === "vani" && [
                  ...
                  <div key={i} onClick={() => { setSelectedCM(v.cm); setTab("commesse"); }} style={{
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:11, color:T.sub }}>{v.cm.code} · {v.tipo_alert==="bloccato" ? (v.note||"").replace("🔴 BLOCCATO: ","") : `${countMisure(v)}/6 misure`}</div>
                    </div>
                    <span style={{ color:T.sub, fontSize:16 }}>›</span>
                  </div>
                ))}
        ...
        {cmSubTab !== "report" && (
          <div style={{ padding: "8px 16px" }}>
            ...
            {[...rilievi].reverse().map((r, idx) => {
              ...
              const vaniMisurati = (r.vani || []).filter(v => countMisure(v) >= 6).length;
  ...
  const renderCMCardCompact = (c) => {
    ...
    const vaniA = getVaniAttivi(c);
    const vaniMis = vaniA.filter(v => countMisure(v) >= 6).length;
    const vaniInc = vaniA.filter(v => { const n=countMisure(v); return n>0&&n<6; }).length;
  ...
  const renderCMDetail = () => {
    ...
    const vaniMisurati = vaniList.filter(v => countMisure(v) >= 6);
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:2 }}>
                      ...
                      {(() => {
                        ...
                        const questoIncompleto = !questoBloccato && countMisure(v) > 0 && countMisure(v) < 6;
  ...
  const renderFasePanel = (c) => {
    ...
    if (c.fase === "sopralluogo") {
      const vaniAttivi2 = getVaniAttivi(c); const vaniCompletati = vaniAttivi2.filter(v => countMisure(v) >= 6).length;
    ...
    if (c.fase === "misure") {
      ...
      const vaniOk = vaniCalc.filter(v => countMisure(v) >= 9).length;
      return (
        (() => {
  ...
  const renderRiepilogo = () => {
    ...
    const vaniR = getVaniAttivi(c); const vaniFilled = vaniR.filter(v=>countMisure(v)>=6).length;
          ...
          <>
            ...
            <div style={S.card}><div style={S.cardInner}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 15, fontWeight: 700 }}>{userFullName}</div>
...
Cordiali saluti,
${userFullName} - ${aziendaInfo.ragione}` },
            ...
            <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setSelectedEvent(null)}>
              ...
              <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", zIndex: 9999, background: T.bg, borderRadius: 16, padding: 20, width: "90%", maxWidth: 420, boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 8 }}>
                  ...
                  <div onClick={() => { const cliente = cmObj ? `${cmObj.cliente} ${cmObj.cognome||""}`.trim() : (ev.persona || "Cliente"); const dataFmt = new Date(ev.date).toLocaleDateString("it-IT", { weekday:"long", day:"numeric", month:"long" }); setMailBody(`Gentile ${cliente},\n\nLe confermo l'appuntamento:\n\n${dataFmt}${ev.time ? " alle " + ev.time : ""}\n${ev.addr || ""}\n\n${ev.text}\n\nCordiali saluti,\n${userFullName}`); setShowMailModal({ ev, cm: cmObj }); setSelectedEvent(null); }} style={{ padding: "12px 4px", borderRadius: 10, background: T.accLt, textAlign: "center", cursor: "pointer", fontSize: 12, fontWeight: 700, color: T.acc }}>{"✉️"} Mail</div>
